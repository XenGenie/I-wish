<ol class="breadcrumb">
  ...
  <li ><a href="/{$toBackDoor}" class="btn btn-xs btn-default"><i class="fa fa-dashboard"></i> Dashboard</a></li>
  {foreach $xtras as $x => $xtra}
      {if $xtra.icon && "x{$Xtra|ucfirst}" == $xtra.class}
        <li class="btn btn-xs btn-default"><a href="/{$toBackDoor}/{$xtra.see}/"><i class="fa fa-{$xtra.see}"></i>
        ...
        <li class="
          {if $method == 'index'} 
            active
          {/if} 
          btn btn-xs btn-default"><a href="/{$toBackDoor}/{$xtra.link}"><i class="fa fa-{$xtra.mini}"></i>
        {$xtra.name} 
        <!-- <label class="badge  -->
